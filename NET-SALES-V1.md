...
| ✅ ĐÃ ĐÓNG — POS wiring + hydration `deps` (2026-09-17) | Đến trước đợt sửa này, **KHÔNG có đường nào trong toàn bộ codebase** cấp `deps.units`/`deps.versionRegistry` thật cho `RecordSale` — `bootstrap/legacy-data-source.js#forCommand` là pass-through VĨNH VIỄN theo thiết kế (đọc comment ở đó), và `app-pos/controller.js#checkout()` còn gửi sai hẳn shape (`{lines, payment}` thay vì `{bill, deps}`). Quyết định chủ quán 2026-09-17 (chat, không phải trong file audit gốc): phạm vi đợt sửa CHỈ `RecordSale` (các command khác — `RecordWaste`/`ReceiveGoods`/prep/... — vẫn đợi cấp `deps` sau, KHÔNG phải gap mới phát sinh, chỉ là gap cũ chưa tới lượt); ngày cutover `2026-09-20` xác nhận KHÔNG hard — chỉ là mốc dự kiến, có thể lùi 21/22 nếu cần. Đã xây: `persistence-firebase/canonical-read-port.js` (đọc canonical Firestore `orgs/{org}/stores/{store}/...` — khác hẳn `bootstrap/firebase-read-client.js` vốn chỉ đọc schema legacy phẳng), `bootstrap/canonical-data-source.js` (hydrate 2 pha: pha 1 đọc version recipe/packaging/iceCogs dựng registry tạm rồi gọi `buildRequirements` THẬT để suy itemId nguyên liệu; pha 2 đọc version cost + Unit của đúng các itemId đó, nạp tiếp vào CÙNG registry), ghép vào `bootstrap/startup.js#composeDataSource` chạy TRƯỚC `legacy-data-source` (không đổi hợp đồng pass-through của legacy, chỉ lấp đúng chỗ nó cố ý chừa trống). `commands/sales.js` thêm `createVersionRegistry()` export (để `bootstrap` dựng registry mà không phải tự import `compaction` — `layer-rules.json` không cho phép). `bootstrap/runtime.js` lộ thêm `context()` ra `api` (trước chỉ dùng nội bộ) để controller tự dựng `buildBill()` đúng `storeId`/`soldByActorId`/`businessDate`/`occurredAt` từ StoreContext hiện tại, không tự đoán. `app-pos/controller.js#checkout(payment, opts)` giờ dựng Bill thật qua `buildBill()`, mặc định `channel: DINE_IN` (chưa có UI chọn kênh — `opts.channel` là chỗ nối khi UI đó được xây), và giải nén bill đã chốt từ `plan.domainRecords` (`type: 'bill'`) gắn lại thành `.bill` để `main.js` đọc `checkoutView.bill.total` không phải đổi. Không có version/Unit cho một itemId → mảng rỗng, KHÔNG lỗi (đúng §2.3a, suy biến qua `gapLines`/`cogsActual: null` đã có sẵn ở N10). Test: `tests/unit/app-shell.test.js` (P9 checkout) cập nhật theo shape `{bill}` mới. |
...
| Điểm nối cho voucher (2026-09-17) | Chủ quán xác nhận: *"Đổi tem only, cắt voucher, nhưng chừa 1 đường ghi chú, nếu sau này có triển khai voucher có thể thêm vào dễ dàng."* Đã chừa 2 điểm nối trong `commands/sales.js` (comment tại chỗ, KHÔNG có code voucher thật nào chạy): (1) validate `redemption.type` trong `buildBill` — thêm case mới (vd `'VOUCHER_CODE'`) cùng chỗ với `'STAMP_FREE_DRINK'`, không cần đổi shape `spec.redemption`; (2) nhánh tiêu thụ trong `RecordSale.execute()` ngay sau khối `redeemStamps` — thêm `else if` gọi module "tiêu thụ" voucher tương ứng, cùng thành/bại với bill giống cơ chế đổi tem. `catalog/promotion.js` cũng đã có sẵn `extraPromotions` cho khuyến mãi/giảm-giá dạng khác (không phải đổi-lấy-sản-phẩm). Không thiết kế thêm gì ngoài 2 điểm nối này — chờ quyết định mở lại rõ ràng nếu/khi cần. |
