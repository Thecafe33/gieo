...
| **Phân loại** | ⚪ **CHƯA HOÀN THÀNH — vẫn là GAP, chỉ đổi hình dạng** |
| **Ghi chú** | Đã đọc trọn `usage-report.js`: nguồn DUY NHẤT là ledger (đúng nguyên tắc — không đọc field tổng đã cache), gộp đúng 3 đường đếm hao hụt cũ (vốn không khớp nhau ở hệ cũ) thành MỘT lần đọc sổ, tách `waste`/`lost`/`found`/`untrackedQty` riêng biệt (không gộp "hao hụt" với "chưa ghi nhận được" làm một, đúng nguyên tắc SEED-CONTRACT). NHƯNG: `build()` nhận `spec.entries` đã lọc theo kỳ ở TẦNG TRÊN, và trả về **một dòng tổng theo itemId cho cả kỳ** — KHÔNG có chiều `businessDate`/`dateKey` trong output (đối chiếu `COLUMNS`: không có cột ngày). So sánh với `reporting/btp-report.js` — module đó CÓ cấu trúc `days[k] = {dateKey, nau, dung, huy, ...}` đầy đủ cho BTP, nhưng chính comment đầu file `btp-report.js` lại nói thẳng: "TÍNH ĐÚNG theo ngày, nhưng chỉ dùng nội bộ cho dự báo và export AI — KHÔNG BAO GIỜ lên màn hình báo cáo cho chủ quán xem", và tự so sánh với đúng lỗi `stockoutTargetPct` ("tính nhưng không bao giờ hiển thị"). Kết luận: (1) nguyên liệu thô — chưa có cấu trúc ngày ở tầng core, muốn có báo cáo ngày phải gọi `GetUsageReport` riêng cho từng ngày ở tầng gọi (khả thi nhưng chưa có UI/wiring nào làm vậy); (2) BTP — CÓ cấu trúc ngày ở tầng core rồi nhưng bị đóng dấu "không hiển thị" ngay trong chính thiết kế mới, tức là **lỗi "tính nhưng giấu" của hệ cũ đang được LẶP LẠI CÓ CHỦ ĐÍCH ở hệ mới cho riêng phần BTP** — cần chủ quán xác nhận đây có đúng là ý định (nội bộ dùng cho AI/dự báo, không phải cho người xem) hay là một sơ hở cần vá. |
...
- ⚪ CHƯA QUYẾT / CHƯA XÁC ĐỊNH (treo, quyết sau khi đủ NET): **RM3** (treo theo RM1), **RM7** (báo cáo ngày — 2 vấn đề con: raw chưa có cấu trúc ngày; BTP có cấu trúc ngày nhưng chủ đích không hiển thị)
...
4. RM7: quyết `btp-report.js`'s dữ liệu ngày ({nau, dung, huy}) — giữ nguyên "chỉ dùng nội bộ, không hiển thị" (chủ đích) hay mở ra màn hình cho chủ quán, vì hiện đang lặp lại đúng mẫu lỗi đã ghi nhận ở `stockoutTargetPct`.
