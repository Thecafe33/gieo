...
| Transaction Drill-down | breakdown → `histSetFilter(type)` lọc đúng loại giao dịch; dòng CONSUMPTION → nút "Xem bill" (`goBillDetail()`) | Nguyên liệu, BTP | thấp | 🟡 **Nâng cấp 2026-09-08** — bấm dòng "Bán ra (GOGS)" trong breakdown giờ hiện TỪNG lượt bán riêng (không gộp theo ngày như mặc định) kèm nút "Xem bill" mở thẳng đúng bill đã tạo ra giao dịch đó (tái dùng `qlOpenBill`/`qlBillDetailHTML` sẵn có ở Lịch sử bill, qua `referenceId=orderId` mà POS đã ghi sẵn). Các loại khác (WASTE/ADJUSTMENT/RECEIVING) vẫn chỉ lọc được theo loại, CHƯA nhảy tới đúng PO/waste-record gốc |
...
**Phase 10 — Transaction Drill-down tới bill gốc**, nhánh có ý nghĩa nhất trong các loại giao dịch (Bán ra/CONSUMPTION — chiếm phần lớn biến động kho hằng ngày):

- Bấm dòng "Bán ra (GOGS)" trong khối "Giải thích tồn kho" (Phase 9) giờ hiện TỪNG lượt bán riêng theo giờ, thay vì gộp chung một dòng "cả ngày" như mặc định — mỗi dòng có nút "🧾 Xem bill" mở thẳng đúng bill đã tạo ra giao dịch đó.
- Dùng lại nguyên `referenceId` mà POS đã ghi sẵn khi trừ kho lúc bán hàng (`applySalesConsumptionPOS` → `applyStockTransaction({..., referenceId: orderId})`, xác nhận qua đọc `posgieo.html`) — không cần thêm field/collection nào.
- Mở bill qua `qlBillDetailHTML`/`qlBillOpenId` — cơ chế ĐÃ CÓ SẴN của Lịch sử bill (protected), chỉ gọi hàm `goBillDetail()` mới để set đúng khoảng ngày + id trước khi vào màn đó, không đụng gì tới UI của Lịch sử bill. Bill đã bị xoá thì tự hiện "Bill không còn tồn tại" — hành vi có sẵn của `qlBillDetailHTML`, không phải viết thêm.
- Chỉ gộp lại theo ngày khi lọc CHÍNH XÁC 'CONSUMPTION'/'SALE' — 4 chip lọc thô có sẵn (Vào kho/Ra kho/Hao hụt/Điều chỉnh) vẫn gộp như cũ, không đổi hành vi mặc định.
- Các loại giao dịch khác (RECEIVING→PO, WASTE→container, ADJUSTMENT→phiếu kiểm kê) CHƯA làm — xem §4.

File thay đổi: `quanlygieo.html` — hàm mới `goBillDetail()` (gần `goBills()`, khoảng dòng 10569‑10583); sửa `_histRender()` phần tính `rows` (không gộp khi filter CONSUMPTION/SALE) và phần vẽ dòng giao dịch (thêm nút "Xem bill").

**Phase 20 — UX Scenario Audit**, xem file riêng `docs/ux-scenario-audit.md` (25 tình huống, đọc code chứ không phải người dùng thật ngồi thử — giới hạn ghi rõ trong file đó). Phát hiện 2 lỗ hổng thật, cả hai đã sửa ngay trong cùng phiên (xem hai mục dưới).

**Lọc "Lịch sử kho" theo tên nhân viên** (từ phát hiện #2 của audit Phase 20) — xem nhanh gần đây một nhân viên đã ghi hao hụt/nhập/điều chỉnh gì:

- Ô lọc mới ở Kho → Lịch sử kho (nguồn "Nguyên liệu") — lọc CLIENT-SIDE trên dữ liệu ĐÃ TẢI (50 giao dịch/10 phiếu kiểm kho/15 phiếu nhận gần nhất TOÀN CỬA HÀNG), không gọi lại Firestore mỗi lần gõ, không phải audit toàn bộ lịch sử của một nhân viên.
- Tách hàm tải dữ liệu (`renderKhoHistItems`, gọi 1 lần khi vào tab) khỏi hàm vẽ lại (`renderKhoHistItemsBody`, gọi lại mỗi lần gõ) — và đặt Ô LỌC ở vỏ ngoài `renderKhoHistory()` (không bị vẽ lại theo từng ký tự gõ), giống cách ô tìm sidebar (`#navSearch`) đã làm — tránh lỗi mất focus con trỏ mỗi lần gõ một chữ (nếu ô lọc nằm trong phần bị vẽ lại, browser tạo lại DOM node mới, con trỏ bị đẩy ra ngoài input).

File thay đổi: `quanlygieo.html` — thêm `khoHistStaffFilter`, `_khoHistItemsCache`, hàm `khoHistSetStaffFilter()`, `renderKhoHistItemsBody()` (khoảng dòng 20694‑20755); sửa `renderKhoHistory()` thêm ô lọc vào vỏ ngoài; sửa `renderKhoHistItems()` chỉ còn tải dữ liệu rồi gọi `renderKhoHistItemsBody()`.

**"Dùng trong công thức nào" — recipe usage ngược** (từ phát hiện #19 của audit Phase 20):

- Khối mới trong sheet 🕐 (cả Nguyên liệu và BTP) — quét toàn bộ `recipes_gieogieo` (mỗi doc keyed theo món, `sizes: {size: [rows]}`, mỗi row có `refType` 'item'/'prep'/'topping' + `itemId`/`prepId`/`toppingId`) tìm dòng nào tham chiếu đúng món/BTP đang xem, rồi tra tên món qua `loadPosMenu('togo')`.
- Phải đọc CẢ collection recipe (Firestore không lập chỉ mục theo itemId ở đây, không lọc được tại tầng query) — không rẻ như các khối khác trong sheet này, nên để **bấm mới tải** ("Tải danh sách"), không tự động chạy mỗi lần mở sheet, giống lý do thẻ Integrity ở Alert Inbox cũng để thủ công.

File thay đổi: `quanlygieo.html` — hàm mới `histLoadRecipeUsage()`, `_histRecipeUsageHtml()` (khoảng dòng 20382‑20441); sửa `_histRender()` chèn khối này.
