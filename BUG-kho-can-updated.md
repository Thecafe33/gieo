...
| 21 | 🔴 Nghiêm trọng | Đổ ly BTP: FIFO allocation không idempotent và không rollback nếu ledger fail | `posgieo.html:9075` |
| 22 | 🔴 Nghiêm trọng | Bắt đầu mẻ: retry/lost-ACK có thể trừ nguyên liệu lần 2 hoặc hoàn nhầm nếu batch set đã commit | `posgieo.html:10600` |
| 23 | 🟠 Vừa | Add-on tăng tồn tiêu hao không có txId cố định / durable retry, có thể lệch FIFO/ledger | `posgieo.html:22961` |
| 24 | 🟠 Vừa | Tìm lại hàng mất: nhiều bước không có stable idempotency, có thể cộng tồn/ledger 2 lần sau retry | `posgieo.html:12265` |
...
# VÒNG 3 — Rà lại code hiện tại trước khi triển khai fix

Ngày rà soát: 2026-09-15

Mục tiêu vòng này không chỉ tìm bug mới mà còn kiểm tra **fix có làm hỏng luồng đang đúng hay tạo lỗi dây chuyền**. Các vị trí dưới đây được đối chiếu trực tiếp với `posgieo(5).html` và `quanlygieo(3).html` hiện tại.

## Trạng thái quan trọng của code hiện tại

Ba cơ chế cốt lõi trong báo cáo vẫn CHƯA được vá trong file hiện tại:

- `quanlygieo.html::applyStockTransaction()` vẫn chỉ cập nhật `currentStock`, chưa cập nhật `untrackedPendingDelta`.
- `submitItemAdjust()` vẫn lấy `oldQty` từ `INVENTORY_ITEMS` cache và truyền `qty: diff`.
- `openWeighPad()` vẫn dùng `opts.unit || goc.unit` và `posVesselUnitFactor(unit) || 1`.

Vì vậy khi triển khai phải coi các patch trong báo cáo là **chưa áp dụng**, không được giả định đã có.

---

## 🔴 BUG 17 — Hoàn kho khi Quản lý xoá bill không ghi `untrackedPendingDelta`

**Vị trí hiện tại:** `quanlygieo.html:10633` (`qlReverseStockForOrder`)

### Hiện trạng

`qlReverseStockForOrder()` hoàn nguyên nguyên liệu bằng transaction riêng:

```js
const next = (Number(doc.data().currentStock) || 0) + gom[itemId];
t.update(ref, { currentStock: next, updatedAt: new Date().toISOString() });
```

và ghi transaction `reversal:true`, nhưng không cập nhật `untrackedPendingDelta`.

### Vì sao nguy hiểm

Đây chính là cùng lớp dữ liệu với BUG 1: nếu phần nguyên liệu được hoàn **không thể quy ngược về unit/tem đang mở**, `currentStock` tăng lên nhưng Unit Engine không có vế nào đại diện khoản tăng đó.

Lần recompute kế tiếp có thể xoá khoản hoàn này.

Đặc biệt hàm POS `_reverseIngredientConsumptionPOS()` đã có semantics `reversalCoverage: 'full' | 'untracked'` và chỉ increment `untrackedPendingDelta` khi không hoàn vào unit được. Logic Management phải cùng nguyên tắc, không được có hai cách hoàn kho khác nhau.

### Giải pháp an toàn

Không copy nguyên logic POS sang Management một cách mù quáng. Trước hết phải xác định transaction gốc của bill có `unitAllocations` hay không:

1. Nếu có allocation và việc hoàn unit thành công 100% → recompute từ unit.
2. Nếu không có allocation / không còn unit để hoàn → cộng `untrackedPendingDelta`.
3. Nếu trạng thái hoàn ambiguous → **không cộng thêm một lần nữa**, đánh dấu cần đối soát và chỉ recompute.
4. Transaction reversal phải có ID cố định theo `(orderId + itemId)` như POS để retry không tạo dòng hoàn thứ hai.

**Không sửa bằng cách đơn giản thêm `FieldValue.increment(gom[itemId])` cho mọi bill**, vì trường hợp đã hoàn đầy đủ vào unit mà còn increment sẽ tạo **double-count khi recompute**.

### Test bắt buộc

```text
Bill có allocation → xoá bill → recompute → tồn không đổi thêm.
Bill không còn unit để hoàn → xoá bill → recompute → khoản hoàn vẫn còn đúng 1 lần.
Xoá bill lại → không phát sinh reversal lần 2.
```

---

## 🔴 BUG 18 — Quản lý chỉnh tồn BTP vẫn có thể bị Unit Engine ghi đè

**Vị trí hiện tại:** `quanlygieo.html:18108` (`submitPrepAdjust`)

### Hiện trạng

Luồng chỉnh tồn BTP hiện làm hai việc tách nhau:

```js
// bước 1
update prep_items.currentStock = newQty

// bước 2
update các prep_batches.qtyRemaining theo diff
```

Nhưng khi cập nhật từng `prep_batches`, code chỉ đổi `qtyRemaining`; không đồng bộ `unitBase` của lô trong Firestore và không cập nhật RTDB tương ứng.

Trong khi Unit Engine coi `prep_batches.unitBase`/RTDB là nguồn cho tồn đang mở và `_ueRecomputeCurrentStock()` tính BTP từ tổng unitBase.

### Chuỗi hậu quả

```text
Quản lý chỉnh BTP 1000 → 1500
        ↓
prep_items.currentStock = 1500
prep_batches.qtyRemaining được phân bổ lại
        ↓
unitBase/RT vẫn còn số cũ
        ↓
POS bán / recompute
        ↓
Unit Engine tính lại từ unitBase
        ↓
currentStock có thể quay về số cũ hoặc lệch
```

Đây là phiên bản BTP của BUG 1 và **không nên coi là cleanup**.

### Giải pháp an toàn

Phải quyết định rõ cơ chế chỉnh BTP:

- Nếu BTP thuộc Unit Engine (`prep_batches` là nguồn thật), adjustment phải cập nhật **RTDB + `prep_batches.unitBase`**, sau đó mới recompute `prep_items.currentStock`.
- Nếu phần điều chỉnh không thể gán vào bất kỳ batch/unit nào, phần đó phải đi vào `prep_items.untrackedPendingDelta`.
- Không được chỉ sửa `qtyRemaining`, vì `qtyRemaining` là field tương thích/báo cáo chứ không phải nguồn duy nhất của Unit Engine.

**Đặc biệt:** không được lấy toàn bộ `newQty` rồi fill lại batch; chỉ áp phần `diff` như code hiện tại đang cố làm, để không làm đảo FIFO của các lô đang hoạt động tốt.

### Test bắt buộc

```text
Có 2 lô BTP đang còn → chỉnh tăng 500 → POS bán 1 lần → recompute.
Kỳ vọng: tổng vẫn = tồn mới - phần bán; không lô nào bị reset oan.

Chỉnh giảm → lô cũ giảm trước; lô mới hơn không bị đụng nếu chưa cần.
```

---

## 🟠 BUG 19 — `submitPrepYieldEdit()` vẫn có cửa double-submit qua hộp xác nhận

**Vị trí hiện tại:** `posgieo.html:11353`

### Hiện trạng

Code kiểm tra `_posSubmitBusy` ở đầu hàm nhưng chỉ đặt cờ **sau** `await posConfirm(...)`:

```js
if (_posSubmitBusy) return;
...
if (!(await posConfirm(...))) return;

_posSubmitBusy = true;
```

Hai lần chạm trước khi hộp xác nhận hoàn tất đều vượt qua check đầu tiên. Nếu cả hai lượt được xác nhận, mỗi lượt có thể chạy `_applyPrepYieldEdit()`.

### Hậu quả

Dù `_applyPrepYieldEdit()` có đọc Firestore tươi, cả hai lượt có thể tuần tự sửa cùng một lô với `newQty`, làm:

- `yieldEdits` có thể ghi nhiều lần.
- ledger `ADJUSTMENT` có thể có nhiều dòng cho cùng một lần sửa.
- thông báo/tem in lại có thể bị chạy hai lần.

### Giải pháp an toàn

Có thể đưa `_posSubmitBusy = true` lên trước mọi `await`, nhưng **không được chỉ làm thế rồi bỏ qua idempotency**.

Khuyến nghị:

1. Giữ validate như hiện tại.
2. Sau khi người dùng bấm "Lưu", claim ngay một `yieldEditClaim` cố định theo `batchId + một phiên chỉnh`.
3. Chỉ lượt claim thành công mới được gọi `_applyPrepYieldEdit()`.
4. Nếu người dùng bấm lại sau khi ACK mất, claim cũ phải khiến lượt sau trở thành no-op.

Nếu muốn patch tối thiểu trước mắt, ít nhất phải đặt busy-lock trước `posConfirm` và khóa nút, nhưng vẫn phải kiểm tra lại ledger/idempotency trước khi coi bug đã đóng.

---

# Các rủi ro triển khai mới phát hiện


## VÒNG 4 — Audit theo SYSTEM-DEPENDENCY-TREE

Ngày rà soát: 2026-09-15

Vòng này không chỉ kiểm tra 20 bug đã biết. Tree được dùng như bản đồ truy ngược:

```text
UI action
 → local mutation function
 → Unit/FIFO allocation
 → Firestore transaction / ledger
 → shared state
 → recompute
 → retry / lost ACK
 → consumer ở app kia
...
Mục tiêu là tìm các mutation có cùng pattern với những bug đã biết nhưng chưa có BUG ID.

### 🔴 BUG 21 — Đổ ly BTP: allocation trước ledger nhưng không có idempotency/rollback đầy đủ

**Vị trí:** `posgieo.html:9075` — `_wastePrepQtyPOS()`; caller `submitDrinkWaste()`.
...
```text
unitEngineAllocateConsumption()
        ↓
runTransaction(prep_items + prep_transactions)
```

nhưng `prep_transactions` dùng `.doc()` ngẫu nhiên, không có `txId` cố định theo một `wasteEventId`.
Nếu allocation RT thành công nhưng transaction Firestore thất bại, code hiện tại không có nhánh hoàn allocations tương ứng như `applyPrepConsumptionPOS()`.
Nếu ACK của transaction bị mất sau khi Firestore đã commit và người dùng thao tác lại, lượt sau có thể allocate FIFO lần nữa và ghi ledger lần nữa.

`submitDrinkWaste()` chỉ có `_posSubmitBusy`, nên chống double-tap trong cùng phiên nhưng **không bảo vệ retry sau lost ACK / reload**.

### Hậu quả

```text
Lần 1: RT -100 → ledger có thể commit
ACK mất
Lần 2: RT -100 → ledger -100 lần nữa
        ↓
BTP tồn vật lý thiếu 200 thay vì 100
```
...
ledger fail thật
→ không rollback
→ mất 100 vật lý, không có ledger
```

### Mức độ
**🔴 Nghiêm trọng** vì đây là mutation trực tiếp vào Unit Engine của BTP.

### Fix an toàn

- Sinh `wasteEventId` ổn định cho đúng một lần đổ ly.
- Mỗi `(wasteEventId + prepId)` có txId cố định.
- Pre-check ledger trước khi allocation.
- Nếu ledger fail thật sau allocation, reverse allocations; nếu không xác minh được trạng thái commit thì **không được tự ý reverse**.
- Nếu transaction đã commit nhưng ACK mất, retry phải trở thành no-op.

Không chỉ thêm busy-lock.

---

### 🔴 BUG 22 — Bắt đầu mẻ: retry/lost-ACK có thể trừ nguyên liệu lần 2 hoặc hoàn nhầm

**Vị trí:** `posgieo.html:10600` — `_startPrepBatchImpl()`.

### Phát hiện

Mỗi lần gọi hàm tạo một:

```js
const batchRef = fstore.collection('prep_batches_gieogieo').doc();
```

và các giao dịch tiêu hao nguyên liệu trong vòng lặp không truyền `txId` cố định theo batch. `referenceId` chỉ là ID batch của lần gọi hiện tại.

Đặc biệt nguy hiểm ở bước:
...
Nếu consume đã commit nhưng ACK/lần `batchRef.set()` không rõ trạng thái, code catch hiện tại có thể đi vào đường rollback mà **không kiểm tra trước batchRef đã thực sự tồn tại hay chưa**. Với lost ACK, thao tác `set()` có thể đã commit nhưng promise vẫn reject.

### Hậu quả

Có hai dạng:

1. Retry toàn bộ → tạo batch mới + trừ nguyên liệu lần 2.
2. `batchRef.set()` đã commit nhưng ACK mất → catch tưởng chưa tạo batch và hoàn nguyên nguyên liệu, trong khi batch `cooking` vẫn tồn tại.

Đây là một **distributed workflow** chứ không phải một transaction đơn.

### Fix an toàn

Dùng stable `batchOperationId`/claim:

```text
start operation claim
        ↓
consume each ingredient with fixed txId
        ↓
create/update ONE fixed batch document
        ↓
completed
```

Trong mọi catch sau một write có thể đã commit:

```text
read-back exact document
→ nếu đã tồn tại: không rollback
→ nếu chắc chắn chưa tồn tại: rollback
→ nếu không xác minh được: giữ trạng thái cần retry/manual review
```

---

### 🟠 BUG 23 — Add-on tăng tiêu hao không có stable txId

**Vị trí:** `posgieo.html:22961` — nhánh `dIng[id] > 0` trong `applyAddonConsumptionPOS()`.

### Phát hiện

Nhánh tăng tiêu hao làm:

```text
unitEngineAllocateConsumption()
→ applyStockTransactionPOS(... referenceId: orderId)
```

nhưng **không truyền `txId`**.

Trong khi cùng một file đã dùng stable `orderId + itemId` cho sale consumption chính. Đây là dependency gap rõ ràng khi đối chiếu hai nhánh.

Ngoài ra toàn bộ `applyAddonConsumptionPOS()` chạy non-blocking sau khi RTDB order đã update và cuối cùng chỉ `console.warn` khi thất bại. Không có durable retry queue cho mutation kho của add-on.

### Hậu quả

- Retry/lost ACK có thể allocate FIFO thêm lần nữa.
- Ledger có thể có dòng thứ hai.
- Order đã cập nhật nhưng stock side-effect thất bại thì không có cơ chế durable để biết và bù.

### Fix an toàn

Stable txId theo `(orderId + addon operation/version + itemId)`; không nên chỉ dùng `orderId + itemId` nếu cùng một order có thể add-on nhiều lần.

Cần phân biệt **lần add-on** bằng `addonId` hoặc revision/version của order.

---

### 🟠 BUG 24 — Tìm lại hàng mất: flow nhiều bước nhưng chưa idempotent end-to-end

**Vị trí:** `posgieo.html:12265` — `submitFoundLostContainer()`.
...
→ applyStockTransactionPOS(ADJUSTMENT +baseQty)
→ update lost report
→ reverse employee deduction
```

Các bước dùng nhiều document/collection khác nhau và mutation stock không truyền `txId` cố định.

Busy-lock chỉ chống double tap trong cùng runtime. Nếu `applyStockTransactionPOS()` đã commit nhưng ACK mất, retry có thể cộng `currentStock` +baseQty lần nữa và ghi thêm một ledger ADJUSTMENT.

Đây còn là workflow cross-app vì Management là nơi duyệt lost report ban đầu.

### Fix an toàn

Stable `foundEventId` theo container + lần chuyển `lost → found_again`.

- stock adjustment dùng txId cố định.
- container transition phải kiểm tra trạng thái hiện tại.
- lost report transition phải idempotent.
- employee deduction reversal phải idempotent.
- Nếu một bước fail, retry tiếp tục từ trạng thái đã hoàn thành, không chạy lại toàn bộ chuỗi.

**BUG 24 là phần cần bổ sung vào BUG 16/12, không thay thế hai bug đó.**

---

## Vòng 4 — Kết luận dependency

Sau khi dùng tree truy ngược các mutation, có một pattern lớn hơn 4 bug mới:

> **Các luồng Unit Engine nguy hiểm nhất không phải ở transaction đơn lẻ, mà ở khoảng giữa `allocate/reverse` và Firestore ledger.**

Các điểm phải ưu tiên kiểm tra tiếp:
...
Nếu hai bước này không có stable operation ID + read-back khi ACK không rõ trạng thái,
`_posSubmitBusy` không đủ bảo vệ hệ thống.

### Checklist audit tiếp theo

- [x] Sale consumption chính — đã có txId + pre-check + rollback path.
- [ ] BTP waste — BUG 21.
- [ ] Start prep batch — BUG 22.
- [ ] Add-on consumption — BUG 23.
- [ ] Found lost — BUG 24.
- [ ] Receiving — đã có `idemKey`/txId, nhưng vẫn phải test failure giữa container và aggregate.
- [ ] Stock count — BUG 13 + approval BUG 7.
- [ ] Bill reversal — BUG 17.
- [ ] BTP adjustment — BUG 18.
- [ ] Receiving correction — BUG 20.


## A. BUG 1 không được sửa độc lập với BUG 11/12/14/17/18

Tất cả đều phụ thuộc invariant:
...
Chỉ thêm `untrackedPendingDelta` ở một caller mà không phân loại phần nào đã có unit đại diện sẽ tạo **double-count**.

Đặc biệt với `RECEIVING` có hàng hư: phần có tem phải nằm ở `sealed`, phần không có tem phải nằm ở `untrackedPendingDelta`; không được ghi cả hai.

## B. BUG 2 phải xử lý stale data, nhưng KHÔNG được biến mọi adjustment thành absolute overwrite

`submitItemAdjust()` là absolute count thật; `approveStockCount()` lại là variance delta tại thời điểm kiểm kê.

Hai semantics này khác nhau và phải giữ riêng. Đừng refactor thành một nhánh `absoluteTo` rồi áp nó cho mọi caller.

## C. `approveStockCount()` phải fail-closed

Hiện tại `Promise.allSettled()` được dùng, sau đó phiếu vẫn update `status:'approved'` dù có dòng thất bại.

Sau khi sửa BUG 7, quy tắc phải là:

```text
0 lỗi → approved
1+ lỗi → không được approved hoàn toàn
```

Nên dùng `partially_applied` hoặc giữ `pending_review` kèm danh sách item lỗi. Không được tự động chạy lại tất cả item đã thành công nếu retry, vì như vậy lại tạo adjustment đôi.

## D. Xoá dụng cụ phải sửa atomicity mềm

BUG 8 hiện đã có cảnh báo/cleanup PREP nhưng code hiện tại vẫn chỉ update `prep_items`, chưa update `inventory_items`.

Khi vá, nếu xoá `VESSEL` thành công nhưng một trong các update `vesselIds` thất bại, phải có cảnh báo bền và cơ chế retry cleanup. Không thể coi `Promise.allSettled()` là thành công hoàn toàn.

---

# Thứ tự triển khai mới — ưu tiên không phá luồng đang đúng

## PHASE 0 — Đóng invariant, chưa sửa UI

1. Ghi rõ nguồn sự thật cho từng loại tồn:
   - `untrackedBase`
   - `untrackedPendingDelta`
   - sealed containers
   - RT/open unit
   - batch `unitBase`
2. Lập bảng caller → phần nào có unit đại diện → phần nào phải untracked.
3. Không đổi behavior bán hàng/kiểm kê/cân ở phase này.

## PHASE 1 — Unit Engine + receiving

4. **BUG 3** — unit/factor bộ cân.
5. **BUG 11** — hàng hư lúc nhận.
6. **BUG 14** — nhận hàng thiếu tem.
7. **BUG 15** — hấp thụ nợ FIFO.

Test recompute sau MỖI bước.

## PHASE 2 — Adjustment và stock count

8. **BUG 1** — chuẩn hóa `untrackedPendingDelta` ở Management.
9. **BUG 2** — absolute adjustment + stale protection.
10. **BUG 13** — chống duplicate stock count bằng busy-lock + idempotency.
11. **BUG 7** — approve fail-closed, không đóng phiếu khi có item lỗi.
12. **BUG 17** — rollback/hoàn kho khi xoá bill phải dùng cùng semantics với POS.
13. **BUG 18** — chỉnh tồn BTP phải đồng bộ unitBase/RT hoặc untracked.

## PHASE 3 — Báo mất

14. **BUG 12** — dựng approval flow.
15. Kiểm tra lại nó không cộng/trừ ngoài semantics BUG 1/11/14/17/18.

## PHASE 4 — Bộ cân / state

16. **BUG 9** — reset mode.
17. **BUG 5** — reset `_pwWeighings` / `_pwChoice`.
18. **BUG 10** — không lưu weigh rỗng.
19. **BUG 4** — `wpDone` không nuốt input invalid.
20. **BUG 6** — khóa trước await.
21. **BUG 19** — khóa/idempotency cho sửa yield.

## PHASE 5 — Cleanup

22. **BUG 8** — cleanup vessel refs của cả PREP + nguyên liệu, có retry cleanup.
23. **BUG 16** — xử lý code chết.
24. Dọn dữ liệu cũ và đối soát sau cùng.

---

# Regression matrix — bắt buộc trước khi đưa bản vá vào chạy thật

| Nhóm | Kịch bản | Không được xảy ra |
|---|---|---|
| Unit | đổi g→kg, cân tem cũ | sai 1000 lần |
| Receiving | hàng tốt + hàng hư | hư bị trừ lần 2 |
| Receiving | nhận số không sinh đủ tem | recompute xoá phần không có tem |
| FIFO | mạng lỗi giữa RT và Firestore | nợ bị trừ 2 lần |
| Adjustment | Management chỉnh → POS bán | adjustment biến mất |
| Adjustment | mở form → POS bán → Lưu | ghi sai do cache |
| Stock count | double tap / mất ACK | 2 phiếu / 2 variance |
| Approval | một item lỗi | phiếu vẫn `approved` |
| Delete bill | hoàn bill có/không có unit | hoàn đôi hoặc biến mất sau recompute |
| BTP adjust | chỉnh tổng → recompute | batch/unit quay về số cũ |
| Yield edit | double tap qua confirm | 2 lần ledger/yield edit |
| Weigh | cân sai ở lượt cuối | đóng pad và mất số im lặng |
| Weigh state | cân → đổi tab → manual | bảng kê cũ không dính vào lần sau |
| Vessel | xoá dụng cụ | ref chết còn trong item/prep |

---

# Quy tắc vàng khi vá

**Không thay đổi một `currentStock` write thành `untrackedPendingDelta` một cách máy móc.**

Trước mỗi patch phải trả lời được 3 câu:

1. Số lượng này hiện được đại diện ở đâu?
2. Sau patch, recompute sẽ lấy nó từ đâu?
3. Nếu thao tác retry / double-tap / mất ACK, lần thứ hai có thể cộng thêm lần nữa không?

Nếu chưa trả lời được cả 3 thì chưa nên merge patch.

---

## 🟠 BUG 20 — Sửa phiếu nhận ghi thừa không atomic giữa tem và tồn kho

**Vị trí hiện tại:** `quanlygieo.html:4119` (`fixRecWizApply`)

### Hiện trạng

Luồng sửa phiếu nhận thực hiện theo thứ tự:

```text
1. Huỷ tem thừa / sửa baseQty của tem
2. Sau đó mới transaction sửa currentStock + ledger RECEIVING
3. Sau đó mới cập nhật receiving_record
```

Nghĩa là Firestore **không có một transaction bao trùm toàn bộ nghiệp vụ**.

Nếu bước 1 thành công nhưng bước 2 thất bại do mạng/quota/tranh chấp, hệ thống có thể rơi vào trạng thái:

```text
Tem đã void / giảm dung tích
        ↓
Kho tổng vẫn giữ số cũ
        ↓
Ledger vẫn giữ số nhận cũ
```

Đây là trạng thái lệch giữa **unit source** và **inventory aggregate**.

### Giải pháp an toàn

Không cố nhét Storage hay các thao tác ngoài Firestore vào cùng transaction. Thay vào đó phải thiết kế theo kiểu **claim + state machine / retry-safe**:

1. Ghi một `correction` có id cố định theo `recId + itemId + correctionVersion` với trạng thái `pending`.
2. Claim correction bằng transaction để hai lần bấm không xử lý hai lần.
3. Áp dụng các bước theo thứ tự nhưng mỗi bước có marker đã hoàn thành.
4. Nếu bước sau lỗi, lần retry tiếp theo tiếp tục từ marker chứ không chạy lại bước đã hoàn thành.
5. Chỉ đánh dấu correction `completed` khi cả tem, ledger, tồn tổng và receiving record đã nhất quán.

**Không sửa đơn giản bằng cách đổi thứ tự 1 ↔ 2**, vì Firestore transaction không rollback được thao tác đã commit bên ngoài transaction.

### Test bắt buộc

```text
Cho bước sửa currentStock fail sau khi tem đã void.
Retry correction.
Kỳ vọng: không void tem lần 2, ledger chỉ sửa 1 lần, tồn cuối đúng.
```
