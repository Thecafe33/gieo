    ...
    return merge.resolve({
      ...
      computeLive: function () {
        ...
        bills.forEach(function (b) {
          ...
          /* Tách theo hình thức thanh toán — bill trả nhiều phần (tính riêng)
             thì từng phần cộng đúng vào hình thức của nó, KHÔNG gộp cả bill
             vào một hình thức. `payment.method` dùng chung enum CASH|BANK với
             `commands/finance.RecordExpense`. Không khai method thì vào
             UNKNOWN — nói rõ thay vì đoán là tiền mặt. */
          (b.payments || []).forEach(function (p) {
            var m = p.method || 'UNKNOWN';
            byPaymentMethod[m] = (byPaymentMethod[m] || 0) + (p.amount || 0);
          });
        });
