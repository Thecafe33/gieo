...
3. ✅ **ĐÃ LÀM**: `autoCloseIfStale()` nay có caller. `commands/shift.js#autoCloseStaleShifts(openShifts, nowTs, opts)` bọc `autoCloseIfStale()` cho từng ca trong `openEmployeeShifts`, tách ra `{stillOpen, closedShifts}`. `commands/business-day.js CloseBusinessDay.execute` gọi hàm này TRƯỚC khi tính `closeDayBlockers` — ca đã thật sự treo quá `DEFAULT_MAX_SHIFT_HOURS` tự đóng (mang `autoClosed`/`needsReview` như mọi auto-close khác) và không còn nằm trong danh sách chặn; ca còn trong ngưỡng (chưa treo) vẫn tiếp tục chặn như cũ, không đổi hành vi đó. Mỗi ca tự đóng phát kèm `employeeShift` domainRecord + event `EmployeeCheckedInStateChanged` (`checkedIn: false`) trong CÙNG mutation plan với `BusinessDayClosed` — atomic, không cần đợi L9 (L9 vẫn chưa có consumer cho event này, xem Liên kết chéo domain #90, không đổi). Tests: `shift-cash.test.js` (`autoCloseStaleShifts — ...` và `CloseBusinessDay — ca treo tự đóng trước khi tính blockers`).
