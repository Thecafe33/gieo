...
describe('store-context/context', function () {
  ...
  test('context pre-check-in không bịa businessDay và vẫn chặn command vận hành', function () {
    var preAuth = assertOk(ctxLib.createContext({
      organizationId: ORG, storeId: STORE, actor: posActor(), source: 'POS'
    }));
    assert.strictEqual(preAuth.businessDate, null);
    assertErr(preAuth.assertOperable('RecordSale'), 'PRECONDITION');
  });
