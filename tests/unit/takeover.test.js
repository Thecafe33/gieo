...
describe('commands/takeover — luật tiếp nhận', function () {
  ...
  test('PIN 4 số của hệ cũ mang sang NGUYÊN VẸN', function () {
    ...
    assert.ok(_tk.ids.isId(emp.actorId, 'actor'), 'nhân viên tiếp nhận phải có actorId để audit');
    assert.strictEqual(emp.storeId, _tk.STORE);
    assert.strictEqual(emp.role, 'POS_OPERATOR', 'legacy thiếu role phải nhận quyền tối thiểu');
  });
